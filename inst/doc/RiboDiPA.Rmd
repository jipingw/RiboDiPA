...
```
The `RiboDiPA()` function outputs a list of genes with their $T$-value, $p$-value, and adjusted $p$-values indicated, stored in the value `gene`, along with other intermediate data objects used in the calculation. In most cases, we expect that users will sort by the adjusted $p$-value in order to see the most significant genes genome-wide, which we show above. Genes YDR049-YDR065 are located within the interval selected for this vignette, and we can clearly see highly significant gene hits with TPI1 and RPS13 (YDR050C and YDR064W, respectively), with $q$-values of `r formatC(result.pip$gene["YDR050C","qvalue"],format="e",digits=2)` and `r formatC(result.pip$gene["YDR064W","qvalue"],format="e",digits=2)`. 
